...
float tansig_approx(float x)
{
    ...
    float abs_x = fabsf(x) / 8.0f;
    abs_x = ci_fmin_f32(abs_x,1.0f);
    abs_x = tansig_table[(int)(200.0f * abs_x + 0.5f)];
    y = ci_fsgnj_f32(abs_x,x);
    return y;
}
...
{
    float f_tmp = ci_fmin_f32(f_src,max);
    f_tmp = ci_fmax_f32(f_tmp,min);
    return f_tmp;
}