...
static SemaphoreHandle_t downlink_mutex;
...
static uint8_t wait_audio_play_idle(uint32_t wait_ms)
{
    ...
    while((SAP_STATE_IDLE != sap_get_state()) && (waited_ms < wait_ms))
    {
        vTaskDelay(pdMS_TO_TICKS(5));
        waited_ms += 5;
    }
    return (SAP_STATE_IDLE == sap_get_state()) ? 1 : 0;
}
...
static void send_frame(uint8_t type, const uint8_t *payload, uint16_t len)
{
    ...
    uint8_t frame_crc = crc8(&frame[2], (uint16_t)(pos - 2));
    frame[pos++] = frame_crc;
...
static void stop_downlink(void)
{
    ...
    was_active = downlink_enabled || downlink_codec_started;
    downlink_enabled = 0;
...
static uint8_t start_downlink(void)
{
    ...
    if(SAP_STATE_IDLE != sap_get_state())
    {
        sap_stop();
...
void ai_uart_i2s_handle_command(const ai_uart_i2s_command_t *cmd)
{
    ...
    case AI_UART_MSG_SET_VOLUME:
    {
        ...
        mprintf("[AUDIO] runtime volume applied requestedPercent=%u promptRatioPercent=%u promptPcmGainPermille=%u persisted=false\r\n",
            ...
            ((unsigned int)requested_percent *
                (unsigned int)ESP_VOLUME_TO_LOCAL_PROMPT_NUMERATOR * 1000U) /
                (unsigned int)ESP_VOLUME_TO_LOCAL_PROMPT_DENOMINATOR);
        break;
    }
...
void ai_uart_i2s_protocol_init(void)
{
    ...
    downlink_mutex = xSemaphoreCreateMutex();
    CI_ASSERT(send_mutex && downlink_mutex, "ai uart resources\r\n");
