...
#if USE_SED_CRY
const sed_param_config_t g_sed_config_para = 
{
...
{  
    float thr = g_sed_config_para.threshold;
#if USE_SED_CRY
    if(g_sed_config_para.times <= TIMES_CRY)
#elif USE_SED_SNORE
    if(g_sed_config_para.times <= TIMES_SNORE)
#endif
    {
        times ++ ;
        if (out_array >= thr)
        ...
        if(times == g_sed_config_para.times )
        {
            if (out_flag > 0)
            {
                sed_relt_flag = true;
